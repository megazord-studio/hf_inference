...
def run_text_generation(spec, dev: str):
    try:
        ...
        if is_gated_repo_error(e): soft_skip("gated model (no access/auth)", "Request access via HF and login."); return
        if is_missing_model_error(e): soft_skip("model not found on Hugging Face"); return
        safe_print_output({"error": "text-generation failed", "reason": repr(e)})
