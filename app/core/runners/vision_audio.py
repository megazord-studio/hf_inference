...
class AudioToAudioRunner(BaseRunner):
    """Simple denoising using spectral gating on input audio_base64."""
    ...
    def predict(self, inputs: Dict[str, Any], options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        buf = _decode_base64_audio(b64)
        data, sr = sf.read(buf)
        if data.ndim > 1:
            data = data.mean(axis=1)
        # Very simple noise gate: zero-out low-energy segments
        energy = np.abs(data)
        ...
        audio_b64 = "data:audio/wav;base64," + _b64.b64encode(out_buf.getvalue()).decode("ascii")
        return {"audio_base64": audio_b64, "sample_rate": sr}


class TextToAudioRunner(BaseRunner):
    """Text-to-audio using a lightweight TTS or simple waveform synthesis."""
...
class AudioTextToTextRunner(BaseRunner):
    """Audio-text-to-text using Wav2Vec2 ASR for transcription only."""
