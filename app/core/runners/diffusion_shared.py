...
from typing import Dict, Any, Optional, Tuple, Type, Protocol
...
def _download_model(model_id: str) -> str:
    ...
    local_dir = snapshot_download(repo_id=model_id, max_workers=16, local_dir=None, local_dir_use_symlinks=True)
    total_bytes = 0
    file_count = 0
    ...
    log.info(f"[sd] download done model={model_id} revision=latest files={file_count} size_mb={mb:.2f} time_s={dur:.2f}")
    return local_dir
...
def _get_or_create_scheduler(local_dir: str, scheduler_cls: Type[_HasFromPretrained]) -> Any:
    ...
    scheduler = from_pretrained(local_dir, subfolder="scheduler", local_files_only=True)
...
def _apply_tiny_unet_weights(pipe_unet: torch.nn.Module, weights_path: str) -> None:
    try:
        ...
        raise RuntimeError("safetensors is required to load tiny SD v1.5 weights.") from exc
    ...
    if missing or unexpected:
        log.info(f"[sd] tiny UNet load_state_dict non-strict: missing={len(missing)} unexpected={len(unexpected)}")

def _bootstrap_tiny_sd15_pipeline(pipe_cls: Type[Any], unet_weights_path: str, device: Optional[torch.device]) -> Any:
    ...
    text_encoder = CLIPTextModel.from_pretrained(base_dir, subfolder="text_encoder", torch_dtype=dtype, local_files_only=True)
    vae = AutoencoderKL.from_pretrained(base_dir, subfolder="vae", torch_dtype=dtype, local_files_only=True)
    unet = UNet2DConditionModel.from_pretrained(base_dir, subfolder="unet", torch_dtype=dtype, local_files_only=True)
    _apply_tiny_unet_weights(unet, unet_weights_path)
    pipe: Any
    if pipe_cls is StableDiffusionImg2ImgPipeline:
        pipe = StableDiffusionImg2ImgPipeline(text_encoder=text_encoder, tokenizer=tokenizer, unet=unet, vae=vae, scheduler=scheduler, safety_checker=None, feature_extractor=None)
    else:
        pipe = StableDiffusionPipeline(text_encoder=text_encoder, tokenizer=tokenizer, unet=unet, vae=vae, scheduler=scheduler, safety_checker=None, feature_extractor=None)
...
def _bootstrap_text_to_video_pipeline(local_dir: str, device: Optional[torch.device]) -> Any:
    from diffusers import TextToVideoSDPipeline, UNet3DConditionModel, DPMSolverMultistepScheduler
    ...
    tokenizer = CLIPTokenizer.from_pretrained(local_dir, subfolder="tokenizer", local_files_only=True)
    text_encoder = CLIPTextModel.from_pretrained(local_dir, subfolder="text_encoder", torch_dtype=dtype, local_files_only=True)
    vae = AutoencoderKL.from_pretrained(local_dir, subfolder="vae", torch_dtype=dtype, local_files_only=True)
    unet = UNet3DConditionModel.from_pretrained(local_dir, subfolder="unet", torch_dtype=dtype, local_files_only=True)
    pipe = TextToVideoSDPipeline(vae=vae, text_encoder=text_encoder, tokenizer=tokenizer, unet=unet, scheduler=scheduler)
    if device and hasattr(pipe, "to"):
        try:
            pipe.to(device)
        except Exception:
...
def _bootstrap_from_model_index(local_dir: str, device: Optional[torch.device]) -> Optional[Any]:
    model_index = _load_model_index(local_dir)
    if not model_index:
        return None
...
def _init_pipeline(pipe_cls: Type[_HasFromPretrained], local_dir_or_id: str, device: Optional[torch.device], *, local: bool) -> Any:
    start = time.time()
    dtype = torch.float16 if (device and device.type in ("cuda", "mps")) else None
    ...
    if local:
        pipe = from_pretrained(local_dir_or_id, local_files_only=True, torch_dtype=dtype)
    else:
...
def get_or_create_sd_pipeline(model_id: str, device: Optional[torch.device], mode: str) -> Dict[str, Any]:
    ...
    if mode == "text" and entry["pipe_text"] is None:
        ...
        if arch_pipe:
            ...
            entry["pipe_text"] = _bootstrap_tiny_sd15_pipeline(StableDiffusionPipeline, tiny_unet_path, device)
        elif use_direct:
            entry["pipe_text"] = _init_pipeline(StableDiffusionPipeline, model_id, device, local=False)
        else:
            if _has_model_index(local):
                entry["pipe_text"] = _init_pipeline(StableDiffusionPipeline, local, device, local=True)
            else:
                entry["pipe_text"] = _init_pipeline(StableDiffusionPipeline, model_id, device, local=False)
    if mode == "img2img" and entry["pipe_img2img"] is None:
        ...
        if arch_pipe:
            ...
            entry["pipe_img2img"] = _bootstrap_tiny_sd15_pipeline(StableDiffusionImg2ImgPipeline, tiny_unet_path, device)
        elif use_direct:
            entry["pipe_img2img"] = _init_pipeline(StableDiffusionImg2ImgPipeline, model_id, device, local=False)
        else:
            if _has_model_index(local):
                entry["pipe_img2img"] = _init_pipeline(StableDiffusionImg2ImgPipeline, local, device, local=True)
            else:
                entry["pipe_img2img"] = _init_pipeline(StableDiffusionImg2ImgPipeline, model_id, device, local=False)
    return entry
