...
class ImageSegmentationRunner(BaseRunner):
    ...
    def load(self) -> int:
        ...
        log.info(
            "vision: loading AutoModelForSemanticSegmentation for %s", self.model_id
        )
    ...
    def _try_load_segformer_processor(self) -> bool:
        """Try to load SegformerImageProcessor with custom config handling."""
        allowed_params = set(
            inspect.signature(SegformerImageProcessor.__init__).parameters.keys()
        ) - {"self"}
