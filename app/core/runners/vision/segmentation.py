...
class ImageSegmentationRunner(BaseRunner):
    ...
    def load(self) -> int:
        ...
        log.info("vision: loading AutoModelForSemanticSegmentation for %s", self.model_id)
    ...
    def _load_processor(self) -> None:
        ...
        if segformer_cls is not None and "segformer" in self.model_id.lower():
            processor_loaded = self._try_load_segformer_processor(segformer_cls)
    ...
    def _try_load_segformer_processor(self, segformer_cls: Any) -> bool:
        ...
        if segformer_cls is None:  # guard import failure
            return False
        allowed_params = set(inspect.signature(segformer_cls.__init__).parameters.keys()) - {"self"}
    ...
    def predict(self, inputs: Dict[str, Any], options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for lbl in np.unique(label_map):
                mask = label_map == lbl
                label_name = self.model.config.id2label.get(int(lbl), str(int(lbl)))
